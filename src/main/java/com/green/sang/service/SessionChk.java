...
public class SessionChk implements HandlerInterceptor{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		HttpSession session = request.getSession();	//session을 읽어서 
		if(session == null || session.getAttribute("id") == null) {	//id가있는지 체크한뒤 없으면
			response.sendRedirect("loginForm"); //loginForm으로 보내
